...
- Track this element は登録済み usage ページを開くか再利用し、その対象タブだけに picker を注入
...
固定 GitHub raw 権限は、ユーザーが Try samples の確認画面で同意した場合だけ使います。取得する `many-ai-usage/main/resources/starter.json` とサンプル文字アイコンは、容量・schema 検証済みの公開設定です。7 provider のうち4件には review 済み taught selector/fingerprint が入り、実行コードやユーザーデータは含まず、eval もしません。
...
- Try samples は認証情報やユーザーデータを付けず、公開 GitHub raw の starter とサンプルアイコンへ GET します
...
- 同じ URL の既存タブが複数あっても選択・再利用された対象タブだけに picker が出ること、複数 metric を連続保存できることを確認
- ページ更新後の fingerprint fallback と Re-teach 導線を確認
- 権限を拒否した場合に needs_permission が表示されることを確認
- 空の初期状態で Try samples を一度キャンセル後、同意して7件が重複なく追加され、4件は review 済み taught track、3件は teach 前のページタイルであることを確認
